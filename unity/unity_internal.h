...
public:
	...
	struct Class final {
		...
		auto GetValue(void* obj, const std::string& name) -> RType { 
			auto field = Get<Field>(name);
			if (!field) return RType();
		...
		auto FindObjectsByType() -> std::vector<T> {
			...
			if (!pMethod) {
				...
				if (!pAssembly) return std::vector<T>(0);
				auto pClass = pAssembly->Get("Object");
				if (!pClass) return std::vector<T>(0);
				pMethod = pClass->Get<Method>("FindObjectsOfType", { "System.Type" });
			}
...
public:
	...
	public:
		...
		struct GameObject : UnityObject {
			...
			template <typename T>
			auto AddComponent(Class* type) -> T {
				if (!type) return T();
				static Method* method;
				if (!method) {
					auto pAssembly = Get("UnityEngine.CoreModule.dll");
					if (!pAssembly) return T();
					auto pClass = pAssembly->Get("GameObject");
					if (!pClass) return T();
					method = pClass->Get<Method>("AddComponent", { "System.Type" });
				}
				if (method) return method->Invoke<T>(this, type->GetType());
				return T();
			}
